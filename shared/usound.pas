...
type
  ...
  tSound = class(tResource)
    ...
    function asFormat(af: tAudioFormat): tSound;
    function clone(): tSound;
    function getSample(pos: int32): tAudioSample;
    procedure setSample(pos: int32;sample: tAudioSample);

    class function createNoise(duration: single): tSound;
    class function Load(filename: string): tSound;
...
type
  ...
  protected
    function getSFXByTag(aTag: string): tSound;
  public
    function addResource(filename: string): tResource; override;
    property items[tag: string]: tSound read getSFXByTag; default;
  end;
...
function tSFXLibrary.getSFXByTag(aTag: string): tSound;
var
  res: tResource;
begin
  res := getByTag(aTag);
  assert(res is tSound);
  result := tSound(res);
end;
...
begin
  ...
  assert((res is tSound) or (res is tLazyResource));
  result := res;
end;
...
constructor tSound.create(aFormat: tAudioFormat; aLength: int32; aTag: string='');
begin
  inherited create();
  length := aLength;
  format := aFormat;
...
function tSound.toString(): string;
begin
  exit(tag);
end;

{returns copy of sound in 16bit-stereo format.}
function tSound.asFormat(af: tAudioFormat): tSound;
var
  i: int32;
begin
  result := tSound.create(af, self.length, self.tag);
  for i := 0 to self.length-1 do
    result.setSample(i, self.getSample(i));
end;

{create a copy}
function tSound.clone(): tSound;
begin
  result := self.asFormat(self.format);
end;

function tSound.getSample(pos: int32): tAudioSample;
begin
...
{bytes per sample}
function tSound.bytesPerSample: int32; inline;
begin
  result := AF_SIZE[format];
end;

{returns the RMS of the sample}
function tSound.calculateRMS: double;
var
  i: int32;
  ll, rr: int32;
...
function tSound.duration: double;
begin
  result := length / 44100;
end;

procedure tSound.setSample(pos: int32; sample: tAudioSample);
begin
  if (pos < 0) or (pos >= length) then begin
    exit;
  end;
...
class function tSound.createNoise(duration: single): tSound;
var
  i: int32;
  numSamples: dWord;
  sample: tAudioSample;
begin
  ...
  result := tSound.create(AF_16_STEREO, numSamples);
...
class function tSound.Load(filename: string): tSound;
var
...
begin
  ...
  if assigned(proc) then begin
    startTime := getSec;
    res := proc(filename);
    if not (res is tSound) then fatal('Resources is of invalid type');
